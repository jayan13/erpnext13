...
class ReceivablePayableReport(object):
	...
	def get_currency_fields(self):
		return ['invoiced', 'paid', 'credit_note', 'outstanding', 'range1',
			'range2', 'range3', 'range4', 'range5', 'range6', 'range7', 'range8', 'range9']
	...
	def set_ageing(self, row):
		...
		if getdate(entry_date) > getdate(self.filters.report_date):
			row.range1 = row.range2 = row.range3 = row.range4 = row.range5 = row.range6 = row.range7 = row.range8 = row.range9 = 0.0

		row.total_due = row.range1 + row.range2 + row.range3 + row.range4 + row.range5 + row.range6 + row.range7 + row.range8 + row.range9
	...
	def get_ageing_data(self, entry_date, row):
		...
		if not (self.filters.range1 and self.filters.range2 and self.filters.range3 and self.filters.range4 and self.filters.range5 and self.filters.range6 and self.filters.range7 and self.filters.range8):
			self.filters.range1, self.filters.range2, self.filters.range3, self.filters.range4, self.filters.range5 = 30, 60, 90, 120, 150, 180, 210, 240

		for i, days in enumerate([self.filters.range1, self.filters.range2, self.filters.range3, self.filters.range4, self.filters.range5, self.filters.range6, self.filters.range7, self.filters.range8]):
	...
	def setup_ageing_columns(self):
		...
		if self.filters.get('range5')==None: 
			self.filters["range5"]= 150
		if self.filters.get('range6')==None: 
			self.filters["range6"]= 180
		if self.filters.get('range7')==None: 
			self.filters["range7"]= 210
		if self.filters.get('range8')==None: 
			self.filters["range8"]= 240
		for i, label in enumerate(["0-{range1}".format(range1=self.filters["range1"]),
			...
			"{range7}-{range8}".format(range7=cint(self.filters["range7"])+ 1, range8=self.filters["range8"]),
			"{range8}-{above}".format(range8=cint(self.filters["range8"])+ 1, above=_("Above"))]):
	...
	def get_chart_data(self):
		...
		for row in self.data:
			...
			if not cint(row.bold):
				values = [row.range1, row.range2, row.range3, row.range4, row.range5, row.range6, row.range7, row.range8, row.range9]
